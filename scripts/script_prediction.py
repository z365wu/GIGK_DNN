...
def GIMK_queue_Example_Comparison(Lmax):  #  This function can be put in the file "GIMK_Queue.py"
    ...
    a, b = 3, 15    ### Then mean interarrival time = (a+b)/2
    arr_rate = 2/(a+b) ### Arrival rate
    DNN_net = NN_for_Queue(
        queue_type='continuous', n_max=n_max, Lmax=Lmax, m_max=m_max, input_dim=input_dim, num_classes=Lmax
    )
    ...
    for K in [1, 2, 3]:  # number of servers   # Include K = 3 with m_max = 10 (see top)
        # Load NN model for each server number K
        ...
        for rho in [0.27, 0.66, 0.95]:
            ...
            if Arrival_moments[0] == 1:
                ...
                for i in range (n_max):
                    Arrival_norm_s2025[i] = Arrival_moments[i]/np.power(a1, i+1)
                    Service_norm_s2025[i] = Service_moments[i]/np.power(a1, i+1)
            ...
            if K == 1: # using Baron2024 method only when K = 1
                ...
                Stat_Dis_B2024 = Baron2024(Arrival_norm_s2025, Service_norm_s2025)
            ...
            figure_path = f'Output/Figures/GIMK/statDist_comparison_GIMK_K{K}_queue_{rho}.png'
            # Save the figure
            ...
            # queue_list = np.maximum(queue_list - np.ones(Lmax)*K, np.zeros(Lmax))
            E_qw_DNNC = np.dot(df_StatDist['DNNC'], queue_list)[0]
            E_qw_DNND = np.dot(df_StatDist['DNND'], queue_list)[0]
            E_qw_DNNM = np.dot(df_StatDist['DNNM'], queue_list)[0]
            E_qw_Sherzer = np.dot(df_StatDist['S(2025)'], queue_list)[0]   
            if K == 1:
                E_qw_Baron = np.dot(df_StatDist['B(2024)'], queue_list)[0]
...
def df_to_latex_GIMK(df):
    ...
    cols = ['DNNC', 'DNND', 'DNNM', 'GIMK', 'Simu', 'S(2025)', 'B(2024)']
    ...
    latex.append(r'\tiny')
    latex.append(r'\caption{Mean queue lengths by $K$, $\rho$, and methods}')
...
if __name__ == '__main__':
    ...
    ######## Section 5, Example 5
    # Part VI: GI/M/K queues.  We consider the mean queue length (not the mean waiting queue length)
