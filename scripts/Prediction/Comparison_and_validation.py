...
def PH_Represent(queue_type, K, m_max, n_max, Lmax, rho_lower, rho_upper, max_moment_bound = 1.0e+30):
    ...
    if queue_type == 1 or queue_type == 'continuous': # generate continuous queue; queue_type = 1 means continuous queue. 
        ...
        alpha_a, T_a = CTPH_Rep_generator(m_a, 1)#rho*K)      # rho<K and mean for interarrival arrival time
        ...
        alpha_s, T_s = CTPH_Rep_generator(m_s, 1/(rho*K)) #1)    # rho=1 and E[S] = 1 for service times 
...
def Compare_StatDist(queue_type, K, m_max, n_max, Lmax, NN_model, rho_lower, rho_upper, max_moment_bound = 1.0e+30):
    ...
    if queue_type == 1 or queue_type == 'continuous': # generate continuous queue; queue_type = 1 means continuous queue. 
        ...
        df_StatDist['Sherzer2025'] = GGC_dist
...
def Compare_Queue_quantity(df_StatDist, Lmax, K, rho, queue_type):
    ...
    if str(queue_type) in ['continuous', '1']:
        q_0_Sherzer2025 = round(float(df_StatDist['Sherzer2025'][0]),3)
        E_qw_Sherzer2025 = np.dot(df_StatDist['Sherzer2025'][K:], queue_list[:(Lmax-K)])[0]
        E_wait_Sherzer2025 = (E_qw_Sherzer2025 / arrival_rate).round(3)
        mse_QBD_Sherzer2025 = np.mean((df_StatDist['Sherzer2025'] - df_StatDist['Sherzer2025']) ** 2).round(3)
        
        if K==1: # include both B(2024) and S(2025)
            ...
            df_predict_compare = pd.DataFrame(columns=['Quantity', 'DNN', 'QBD', 'Simul', 'W(1993)', 'S(2025)','B(2024)', 'rho'])
        else: # include S(2025)
            df_predict_compare = pd.DataFrame(columns=['Quantity', 'DNN', 'QBD', 'Simul', 'W(1993)', 'S(2025)', 'rho'])
    ...
    if str(queue_type) in ['continuous', '1']: # using S(2025) method only for continuous queue 
        df_predict_compare['S(2025)'] = [q_0_Sherzer2025, E_qw_Sherzer2025.round(3), E_wait_Sherzer2025, mse_QBD_Sherzer2025]
        if  K == 1: # using Baron2024 method only for continuous queue with K = 1
        ...
        for rho_test in Rho_list:
            for i in range(1, batch_size + 1):
                ...
                if queue_type == 'continuous':
                    SAE_Sherzer2025 = np.abs(df_StatDist['QBD'] - df_StatDist['Sherzer2025']).sum()
                    if K == 1:
                ...
                if queue_type == 'continuous':
                    REM_Sherzer2025 = np.abs(np.dot(queue_list, (df_StatDist['QBD']) - df_StatDist['Sherzer2025'])) / np.dot(queue_list, df_StatDist['Sherzer2025'])
                    if K == 1:
...
def df_quantity_Compare_to_latex(df, queue_type):
    ...
    for K in unique_K:
        ...
        if str(queue_type) in ['continuous', '1']:
            if K == 1:
                df_pivot = df_K.pivot(index="Quantity", columns="rho", values=["DNN", "QBD", "Simul", "W(1993)", "S(2025)", "B(2024)"])
            else:
                df_pivot = df_K.pivot(index="Quantity", columns="rho", values=["DNN", "QBD", "Simul", "W(1993)", "S(2025)"])
        else:
        ...
        # if str(queue_type) in ['continuous', '1']:
        #     if K == 1:
        #         header_methods = " & " + " & ".join(["B(2024) & DNN & S(2025) & QBD & Simul & W(1993)"] * len(df_pivot.columns.levels[1])) + " \\\\\n"
        #     else:
        #         header_methods = " & " + " & ".join(["DNN & S(2025) & QBD & Simul & W(1993)"] * len(df_pivot.columns.levels[1])) + " \\\\\n"
        # else:
        ...
        # Dynamically detect methods and rhos from pivot
        methods = list(df_pivot.columns.levels[0])
        rhos = list(df_pivot.columns.levels[1])
        n_methods = len(methods)
        n_rhos = len(rhos)
        
        # Build LaTeX headers dynamically
        header_rho = " & " + " & ".join(
            [f"\\multicolumn{{{n_methods}}}{{c}}{{$\\rho = {rho}$}}" for rho in rhos]
        ) + " \\\\\n"
        
        header_methods = " & " + " & ".join([" & ".join(methods)] * n_rhos) + " \\\\\n"
        
        # Convert DataFrame to LaTeX format
        latex_body = df_pivot.to_latex(float_format="%.3f", escape=False, header=False)
        
        # Remove the unwanted "Quantity" row from LaTeX output
        latex_body = "\n".join(latex_body.splitlines()[1:])
        
        # Clean LaTeX table formatting
        latex_body = latex_body.replace("\\toprule", "\\hline") \
                               .replace("\\midrule", "") \
                               .replace("\\bottomrule", "")
        
        # Build tabular column spec dynamically
        col_spec = "l" + "r" * (n_methods * n_rhos)
        ...
        latex_table = (
            ...
            header_methods +
            "\\hline\n"
            + "\n".join(latex_body.splitlines()[2:-1]) +
            "\n\\hline\n"
            "\\end{tabular}\n"
            f"\\label{{tab:Queueing_Quantity_Comparison_K_{K}}}\n"
            "\\end{table}\n\n"
...
def Out_space_samples(queue_type, K, m_max, n_max, Lmax, DNN_model, c_a_2_ubound, c_s_2_ubound, out_sample = True, rho_lower=0, rho_upper=1, max_moment_bound = 1.0e+30):
    ...
    df_StatDist['Simulation'] = Simu_StatDist[:Lmax]
    df_StatDist['Whitt1993'] = StatDistWhitt1993
    if queue_type == 'continuous': # using Baron2024 method only when K = 1
        df_StatDist['Sherzer2025'] = Sherzer2025_dist
        if K == 1:
            df_StatDist['Baron2024'] = Baron2024_dist
    ...
    df_StatDist.iloc[0:20,].plot(kind='bar', figsize=(10, 6), rot=0)
