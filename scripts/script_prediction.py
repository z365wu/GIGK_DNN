...
def GIMK_queue_Example_Comparison(Lmax):  #  This function can be put in the file "GIMK_Queue.py"
    ...
    for K in [1, 2]:  # number of servers   # Include K = 3 with m_max = 10 (see top)
        ...
        for rho in [0.33, 0.66]:
            ...
            '''
            At the end of this function, we want 
               i) Figures: Plot stationary distributions generated by GIMK, Simulation, DNNC, DNND, DNNM, S(2025) and B(2024) for K = 1, 2, 3, and rho = 0.33, 0.66, 0.95
               ii) Tables:   For Mean queue length E[q]
        
                                          |  GIMK | Simu |  DNNC |  DNND |  DNNM | S(2025) | B(2024)
                          K = 1 |rho=0.33 |  XX   |  XX  |   xX  |  XX   |   XX  |  XX    |    XX
                                |rho=0.66 | ...
                                |rho=0.95 | ...
                          K = 2 |rho=0.33 |   XX  |  XX  |   XX  |  XX   |  XX  |   XX    |  --
                                 ...
                          K = 3 ...
            '''
