...
class NN_for_Queue: 
    ...
    def training(self, K):
        ...
        self.train_history = self.model.fit(Moments_train, Stationary_train, batch_size= 1024, epochs= 400, verbose=1)  
    ...
    def Plot_DNN_training_accuracy_loss(self, K_list):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title(f'Training Loss over Epochs for $K=1, 2, 3$, DNNs for {self.queue_type} queues')
        plt.legend()
        plt.grid(True)
        ...
        plt.title(f'Training Accuracy over Epochs for each $K$: {self.queue_type} queue')
        plt.legend()
        plt.grid(True)
    ...
    '''
    #### Need to add mixed queue
    def Training_using_new_samples(self, df_new):
    
        # Obtain all samples for the queue with K servers
        Moments_new, Stationary_new, rho_new = Load_Samples_from_file(self.n_max, self.Lmax, df_new)
        Moments_new = Moments_new.astype(float)
        Stationary_new = Stationary_new.astype(float)
        rho_new = rho_new.astype(float)
        
        # Normalize the moments for discrete queue
        print(Moments_new[:, self.n_max])
        if str(self.queue_type) in ['discrete', '0']:
            Moments_new = Moments_new / Moments_new[:, self.n_max][:, None]  # normalize by dividing the 1st moment of service times
        
        st = time.time()    # Record the total computing time
        # Train or retrain the DNN model
        # Coding model, selection, and loss function 
        self.model.fit(Moments_new, Stationary_new, batch_size= 128, epochs= 1)  
        # Print the training time
        et = time.time()
        Elapsed_time = et - st
        print('Execution time is ', Elapsed_time, 'seconds.')
    ...
    def Plotting_queue_length_distribution(self, K, NN_model, rho_lower, rho_upper, save_fig= False, fig_save_name = None):
        ...
        self.df_StatDist.iloc[0:30,].plot(kind='bar', figsize=(10, 6))
        
        # Adding title and labels
        plt.title(f'Queue length distribution: $K$={K}, $\\rho = {rho}$, {self.queue_type}', fontsize=16)
